...
using dotNES.Renderers.Direct3D;
using dotNES.Settings;
using dotNES.Win32.Renderers;
...
namespace dotNES.Win32
{
    public partial class UI : Form
    {
        ...
        class SeparatorItem : ToolStripMenuItem
        {
            public SeparatorItem() : base("-") { }
        }

        class Item : ToolStripMenuItem
        {
            ...
            public void Add(ToolStripMenuItem item) => this.DropDownItems.Add(item);
        }
        ...
        {
            // ToLower in case a system is case sensitive, should be lowercase.
            if(Path.GetExtension(rom).ToLower() == ".zip")
            {
                var romFile = Path.GetFileNameWithoutExtension(rom) + ".nes";
                var tempDir = System.IO.Path.GetTempPath();
                var renamedRom = Path.Combine(tempDir, romFile);

                // Extract and correct path.
                if (!File.Exists(renamedRom))
                {
                    try
                    {
                        ZipFile.ExtractToDirectory(rom, tempDir);
                        rom = renamedRom;
                    }
                    catch (Exception ex)
                    {
